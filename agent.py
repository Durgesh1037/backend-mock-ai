...
async def delete_proctor_session(session_id: str):
    ...
    try:
        ...
        if session_path.exists():
            ...
            logger.info(f"🗑️  Deleted proctoring session: {session_id}")
            return {"success": True, "message": f"Proctoring session {session_id} deleted"}
        else:
            logger.warning(f"⚠️  Proctoring session not found: {session_id}")
...
def start_api_server_in_background() -> None:
    ...
    def _load_model():
        ...
        time.sleep(3)
        logger.info("🔄 Loading proctoring model in BACKGROUND thread...")
        try:
            load_proctoring_model()
            logger.info("✅ Proctoring model loaded in background!")
        except Exception as e:
            logger.error(f"❌ Failed to load proctoring model in background: {e}")
    ...
    m.start()
    logger.info("✅ Model loading thread started (wont block startup)")

async def entrypoint(ctx: agents.JobContext):
    # Log room details
    logger.info(f"🏠 Joining room: {ctx.room.name if ctx.room else 'Unknown'}")
    logger.info(f"🔌 Connected to LiveKit server")
    ...
    if candidate_data:
        ...
        logger.warning("⚠️  No candidate data found - using default interview mode")
...
if __name__ == "__main__":
    try:
        ...
        logger.info("🚀 Starting SkillCef agent...")
        agents.cli.run_app(
